...
A **prefix sum** (a.k.a. cumulative sum) pre-computes running totals so that the sum of any range can be answered in **O(1)** — no matter how large the range.
...
- **2D**: `p[i][j]` stores the sum of the rectangle from `(0,0)` to `(i-1,j-1)`. Uses **inclusion–exclusion**:
