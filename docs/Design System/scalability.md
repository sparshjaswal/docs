...
> **Golden rule**: Push state *out* of your app servers. Store session data in a shared store (Redis, a database, or a signed token like a JWT) so app servers stay stateless and disposable.
...
Back-of-the-envelope math justifies every scaling decision. Estimate the load *before* choosing a design.
