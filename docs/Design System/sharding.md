...
- Complements [replication](./replication.md) (which scales *reads* and availability) — sharding scales *writes*.
...
> ❌ *Bad key*: `country` (few values, skewed). ✅ *Good key*: `user_id` (high cardinality, evenly hashed).
...
- Avoid `hash % N` for a variable `N` — changing `N` remaps *most* keys.
...
- The standard technique for dynamic sharding & distributed caches. See also [Hashing](../hashing/README.md).
