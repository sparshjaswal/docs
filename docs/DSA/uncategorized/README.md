---
id: dsa-uncategorized-readme
title: 🧩 Uncategorized Problems
slug: /DSA/uncategorized/README
sidebar_label: 🧩 Uncategorized Problems
---

# 🧩 Uncategorized Problems

> **One-line summary**: Well-known algorithmic problems that span multiple paradigms (backtracking, DP, greedy, divide-and-conquer) and don't fit a single category.

---

## Problems in this section

Runnable implementations (with tests) live alongside this guide in this folder:

- [Tower of Hanoi](./hanoi-tower/)
- [Square Matrix Rotation](./square-matrix-rotation/)
- [Jump Game](./jump-game/)
- [Unique Paths](./unique-paths/)
- [Rain Terraces](./rain-terraces/)
- [Recursive Staircase](./recursive-staircase/)
- [Best Time To Buy/Sell Stocks](./best-time-to-buy-sell-stocks/)
- [N-Queens Problem](./n-queens/)
- [Knight's Tour](./knight-tour/)

---

## Related Topics

- [Backtracking](../backtracking/README.md) — N-Queens, Knight's Tour
- [Dynamic Programming](../dp/README.md) — jump game, unique paths, rain terraces

[← Back to Home](../index.md) · © sparshjaswal
